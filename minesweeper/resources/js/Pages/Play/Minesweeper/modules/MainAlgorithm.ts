...
export const toggleFlag = (row: number, column: number) => {
    ...
    if (interval === 0) {
        interval = window.setInterval(advanceTimer, 1000, interval);
    }
    ...
    const remainsCounter = document.getElementsByClassName("mine-counter")[0];
...
export const touchCell = (
    ...
    if (hasOpenedMinedCell || hasOpenedAllSafeCells) {
        ...
        if (hasOpenedMinedCell) {
            ...
            for (let row = 0; row < rowSize; row++) {
                for (let column = 0; column < columnSize; column++) {
                    if (
                        ...
                        const minedCell = document.getElementById(
                            `cell-${row}-${column}`
                        );
                        if (minedCell === null) {
                            continue;
            ...
            const remainsCounter =
                document.getElementsByClassName("mine-counter")[0];
